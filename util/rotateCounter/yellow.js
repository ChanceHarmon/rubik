...
module.exports = function rotateYellowCounterClockwise(str) {
                ...
                .then(() => {
                  client.query(sql5)
                    ...
                    .then(() => {
                      ...
                      let sql = `UPDATE red SET positions='${travelLogArray[2][2].positions.split(',')[0]},${travelLogArray[2][2].positions.split(',')[1]},${travelLogArray[2][2].positions.split(',')[2]}' WHERE id=3;`;
                      client.query(sql)
                        .then(() => {
                          let sql = `UPDATE green SET positions='${travelLogArray[0][2].positions.split(',')[0]},${travelLogArray[0][2].positions.split(',')[1]},${travelLogArray[0][2].positions.split(',')[2]}' WHERE id=3;`;
                          client.query(sql)
                            .then(() => {
                              let sql = `UPDATE blue SET positions='${travelLogArray[3][2].positions.split(',')[0]},${travelLogArray[3][2].positions.split(',')[1]},${travelLogArray[3][2].positions.split(',')[2]}' WHERE id=3;`;
                              client.query(sql)
                                .then(() => {
                                  let sql = `UPDATE orange SET positions='${travelLogArray[1][2].positions.split(',')[0]},${travelLogArray[1][2].positions.split(',')[1]},${travelLogArray[1][2].positions.split(',')[2]}' WHERE id=3;`;
                                  client.query(sql)
                                    .then(() => {
                                      return travelLog;
                                    })
